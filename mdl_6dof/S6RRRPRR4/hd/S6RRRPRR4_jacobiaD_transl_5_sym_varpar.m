...
function JaD_transl = S6RRRPRR4_jacobiaD_transl_5_sym_varpar(qJ, qJD, r_i_i_C, ...
  pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(6,1),zeros(6,1),zeros(3,1),zeros(11,1)}
assert(isreal(qJ) && all(size(qJ) == [6 1]), ...
  'S6RRRPRR4_jacobiaD_transl_5_sym_varpar: qJ has to be [6x1] (double)');
assert(isreal(qJD) && all(size(qJD) == [6 1]), ...
  'S6RRRPRR4_jacobiaD_transl_5_sym_varpar: qJD has to be [6x1] (double)');
assert(isa(r_i_i_C,'double') && isreal(r_i_i_C) && all(size(r_i_i_C) == [3 1]), ...
	'S6RRRPRR4_jacobiaD_transl_5_sym_varpar: Position vector r_i_i_C has to be [3x1] double');
assert(isreal(pkin) && all(size(pkin) == [11 1]), ...
  'S6RRRPRR4_jacobiaD_transl_5_sym_varpar: pkin has to be [11x1] (double)');
...
t254 = cos(pkin(11)) * pkin(4) + pkin(3);
...
t264 = -pkin(9) - qJ(4);
t307 = r_i_i_C(3) - t264;
t317 = (-t254 * t258 + t307 * t259) * t261 + (sin(pkin(11)) * pkin(4) + pkin(8) + pkin(7)) * qJD(1) + t253 - t295;
t289 = t256 * t297;
