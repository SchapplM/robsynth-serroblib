...
function [tau, v_i_i_ges, w_i_i_ges] = S6RRRRRR10_invdyn_floatb_eulxyz_nnew_vp1(q, qD, qDD, phi_base, xD_base, xDD_base, ...
...
assert(isreal(q) && all(size(q) == [6 1]), ...
  'S6RRRRRR10_invdyn_floatb_eulxyz_nnew_vp1: q has to be [6x1] (double)');
assert(isreal(qD) && all(size(qD) == [6 1]), ...
  'S6RRRRRR10_invdyn_floatb_eulxyz_nnew_vp1: qD has to be [6x1] (double)');
assert(isreal(qDD) && all(size(qDD) == [6 1]), ...
  'S6RRRRRR10_invdyn_floatb_eulxyz_nnew_vp1: qDD has to be [6x1] (double)');
assert(isreal(phi_base) && all(size(phi_base) == [3 1]), ...
  'S6RRRRRR10_invdyn_floatb_eulxyz_nnew_vp1: phi_base has to be [3x1] (double)');
assert(isreal(xD_base) && all(size(xD_base) == [6 1]), ...
  'S6RRRRRR10_invdyn_floatb_eulxyz_nnew_vp1: xD_base has to be [6x1] (double)');
assert(isreal(xDD_base) && all(size(xDD_base) == [6 1]), ...
  'S6RRRRRR10_invdyn_floatb_eulxyz_nnew_vp1: xDD_base has to be [6x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [14 1]), ...
  'S6RRRRRR10_invdyn_floatb_eulxyz_nnew_vp1: Kinematic parameters pkin have to be [14x1] (double)');
assert(isreal(m_num) && all(size(m_num) == [7 1]), ...
  'S6RRRRRR10_invdyn_floatb_eulxyz_nnew_vp1: m_num has to be [7x1] (double)');
assert(isreal(rSges_num_mdh) && all(size(rSges_num_mdh) == [7,3]), ...
  'S6RRRRRR10_invdyn_floatb_eulxyz_nnew_vp1: rSges_num_mdh has to be [7x3] (double)');
assert(isreal(Icges_num_mdh) && all(size(Icges_num_mdh) == [7 6]), ...
  'S6RRRRRR10_invdyn_floatb_eulxyz_nnew_vp1: Icges_num_mdh has to be [7x6] (double)');
