...
%   Siehe auch: S6RRRRRR10_fkine_fixb_rotmat_mdh_sym_varpar.m
% r_i_i_C [3x1]
...
function JgD_C = S6RRRRRR10_jacobigD_mdh_eulxyz_num(phi_base, phiD_base, qJ, qJD, link_index, r_i_i_C, ...
  pkin)
%% Init
%#codegen
%$cgargs {zeros(3,1),zeros(3,1),zeros(6,1),zeros(6,1),
%$cgargs  uint8(zeros(1,1)),zeros(3,1),zeros(14,1)}
assert(isreal(phi_base) && all(size(phi_base) == [3 1]), ...
  'S6RRRRRR10_jacobigD_mdh_eulxyz_num: Base Euler angles have to be [3x1] (double)');
assert(isreal(phiD_base) && all(size(phiD_base) == [3 1]), ...
  'S6RRRRRR10_jacobigD_mdh_eulxyz_num: Base Euler angles time derivatives have to be [3x1] (double)');
assert(isreal(qJ) && all(size(qJ) == [6 1]), ...
  'S6RRRRRR10_jacobigD_mdh_eulxyz_num: Joint angles qJ have to be [6x1] (double)');
assert(isreal(qJD) && all(size(qJD) == [6 1]), ...
  'S6RRRRRR10_jacobigD_mdh_eulxyz_num: Joint velocities qJD have to be [6x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
  'S6RRRRRR10_jacobigD_mdh_eulxyz_num: link_index has to be [1x1] uint8');
assert(isreal(r_i_i_C) && all(size(r_i_i_C) == [3 1]), ...
  'S6RRRRRR10_jacobigD_mdh_eulxyz_num: Position vector r_i_i_C has to be [3x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [14 1]), ...
  'S6RRRRRR10_jacobigD_mdh_eulxyz_num: Kinematic parameters pkin have to be [14x1] (double)');
...
%% Kinematik berechnen
T_c_mdh = S6RRRRRR10_fkine_fixb_rotmat_mdh_sym_varpar(qJ, pkin);
v_mdh = S6RRRRRR10_structural_kinematic_parameters();
...
Jg_JC_B = S6RRRRRR10_jacobig_mdh_num(qJ, link_index, r_i_i_C, pkin);
JgD_JC_B = S6RRRRRR10_jacobigD_mdh_num(qJ, qJD, link_index, r_i_i_C, pkin);
