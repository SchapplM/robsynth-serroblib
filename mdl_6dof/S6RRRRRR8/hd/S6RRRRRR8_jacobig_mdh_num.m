...
function Jg_C = S6RRRRRR8_jacobig_mdh_num(qJ, link_index, r_i_i_C, pkin)
%% Init
%#codegen
%$cgargs {zeros(6,1),uint8(zeros(1,1)),zeros(3,1),zeros(13,1)}
assert(isreal(qJ) && all(size(qJ) == [6 1]), ...
  'S6RRRRRR8_jacobig_mdh_num: Joint angles qJ have to be [6x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
  'S6RRRRRR8_jacobig_mdh_num: link_index has to be [1x1] uint8');
assert(isreal(r_i_i_C) && all(size(r_i_i_C) == [3 1]), ...
  'S6RRRRRR8_jacobig_mdh_num: Position vector r_i_i_C has to be [3x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [13 1]), ...
  'S6RRRRRR8_jacobig_mdh_num: Kinematic parameters pkin have to be [13x1] (double)');
...
[v_mdh, sigma_mdh] = S6RRRRRR8_structural_kinematic_parameters();
