...
function H = S6RRRPPR10_inertiaJ_nCRB_vp1(q, pkin, m_num, rSges_num_mdh, Icges_num_mdh)
%%Coder Information
%#codegen
%$cgargs {zeros(6,1),zeros(11,1),zeros(7,1),zeros(7,3),zeros(7,6)}
assert(isreal(q) && all(size(q) == [6 1]), ...
  'S6RRRPPR10_inertiaJ_nCRB_vp1: q has to be [6x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [11 1]), ...
  'S6RRRPPR10_inertiaJ_nCRB_vp1: Kinematic parameters pkin have to be [11x1] (double)');
assert(isreal(m_num) && all(size(m_num) == [7 1]), ...
  'S6RRRPPR10_inertiaJ_nCRB_vp1: m_num has to be [7x1] (double)');
assert(isreal(rSges_num_mdh) && all(size(rSges_num_mdh) == [7,3]), ...
  'S6RRRPPR10_inertiaJ_nCRB_vp1: rSges_num_mdh has to be [7x3] (double)');
assert(isreal(Icges_num_mdh) && all(size(Icges_num_mdh) == [7 6]), ...
  'S6RRRPPR10_inertiaJ_nCRB_vp1: Icges_num_mdh has to be [7x6] (double)');

%% Init
[v_mdh, sigma] = S6RRRPPR10_structural_kinematic_parameters();
...
%% Kinematik
T_mdh = S6RRRPPR10_joint_trafo_rotmat_mdh_sym_varpar(q, pkin);
