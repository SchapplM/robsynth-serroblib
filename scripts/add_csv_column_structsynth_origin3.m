...
% Die zusätzliche Spalte ist bezogen auf serielle Ketten mit Schubgelenken
...
for N = 1:7
  ...
  for d = dir(fullfile(mdldir, '*.csv'))'
    if contains(d.name, 'copy'), continue; end
    ...
    while ischar(tline)
      ...
      % bereits erfolgt ist. Vorher waren es 5 Herkunftsspalten
      csv_length_old = 1 + 8*N + 3 + 9 + 2 + 5;
      ...
      csvline_new = {csvline{:}, cols_add{:}}; %#ok<CCAT>
