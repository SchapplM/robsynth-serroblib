...
function [q, Phi] = S3PPR1_invkin_eulangresidual(xE_soll, q0, s)

%% Coder Information
%#codegen
%$cgargs {zeros(6,1),zeros(3,1),struct(
%$cgargs            'pkin', zeros(3,1),
%$cgargs          'sigmaJ', zeros(3,1),
%$cgargs             'NQJ', 0,
%$cgargs            'qlim', zeros(3,2),
...
for rr = 0:retry_limit % Schleife über Neu-Anfänge der Berechnung
  ...
  for jj = 2:n_max % Schleife über iteratives Annähern mit Newton-Verfahren
    ...
    % Gradienten-Matrix
    Jdk_voll = S3PPR1_constr2grad(q1, xE_soll, pkin, T_N_E, phiconv_W_E, I_EElink, reci);
    % Zwangsbedingungen
    Phi_voll = S3PPR1_constr2(q1, xE_soll, pkin, T_N_E, phiconv_W_E, I_EElink, reci);
    ...
    if nsoptim && jj < n_max-10 % die letzten Iterationen sind zum Ausgleich des Positionsfehlers (ohne Nullraum)
      ...
      % [1], Gl. (24)
      delta_q_N = (eye(NQJ) - pinv(Jdk)* Jdk) * v;
    end
