...
assert(isreal(qJ) && all(size(qJ) == [3 1]), ...
  'S3PPP1_jacobigD_sym_varpar: qJ has to be [3x1] (double)');
assert(isreal(qJD) && all(size(qJD) == [3 1]), ...
  'S3PPP1_jacobigD_sym_varpar: qJD has to be [3x1] (double)');
assert(isa(r_i_i_C,'double') && isreal(r_i_i_C) && all(size(r_i_i_C) == [3 1]), ...
	'S3PPP1_jacobigD_sym_varpar: Position vector r_i_i_C has to be [3x1] double');
