...
function H = S3PRP1_inertiaJ_nCRB_vp1(q, pkin, m_num, rSges_num_mdh, Icges_num_mdh)
%%Coder Information
%#codegen
%$cgargs {zeros(3,1),zeros(3,1),zeros(4,1),zeros(4,3),zeros(4,6)}
assert(isreal(q) && all(size(q) == [3 1]), ...
  'S3PRP1_inertiaJ_nCRB_vp1: q has to be [3x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [3 1]), ...
  'S3PRP1_inertiaJ_nCRB_vp1: Kinematic parameters pkin have to be [3x1] (double)');
assert(isreal(m_num) && all(size(m_num) == [4 1]), ...
  'S3PRP1_inertiaJ_nCRB_vp1: m_num has to be [4x1] (double)');
assert(isreal(rSges_num_mdh) && all(size(rSges_num_mdh) == [4,3]), ...
  'S3PRP1_inertiaJ_nCRB_vp1: rSges_num_mdh has to be [4x3] (double)');
assert(isreal(Icges_num_mdh) && all(size(Icges_num_mdh) == [4 6]), ...
  'S3PRP1_inertiaJ_nCRB_vp1: Icges_num_mdh has to be [4x6] (double)');

%% Init
[v_mdh, sigma] = S3PRP1_structural_kinematic_parameters();
...
%% Kinematik
T_mdh = S3PRP1_joint_trafo_rotmat_mdh_sym_varpar(q, pkin);
...
I_c_ges = NaN(3,36);
for i = 1:3 % loop through all bodies, [Featherstone2008] T6.2, Line 2
...
for i = 3:-1:1 % Wird die Schleife mit uint8 definiert, funktioniert der generierte mex-Code nicht (Matlab-Bug)
  ...
  for tmp = 1:3 % Dummy-Schleife für Kompilierbarkeit. Abbruchkriterium s.u.
