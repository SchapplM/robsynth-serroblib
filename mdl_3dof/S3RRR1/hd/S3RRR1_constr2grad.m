...
function Phi_dq = S3RRR1_constr2grad(q, xE, pkin, T_N_E, phiconv_W_E, I_EElink, reci)

%% Coder Information
%#codegen
%$cgargs {zeros(3,1),zeros(6,1),zeros(5,1),zeros(4,4),uint8(2),uint8(0), true}

%% Translatorisch
% Bein-Jacobi
J0_i_trans = S3RRR1_jacobia_transl_sym_varpar(q, I_EElink, T_N_E(1:3,4), pkin);
...
Tc_ges = S3RRR1_fkine_fixb_rotmat_mdh_sym_varpar(q, pkin);
...
dRb_0N_dq = S3RRR1_jacobiR_rot_sym_varpar(q, I_EElink, pkin);
