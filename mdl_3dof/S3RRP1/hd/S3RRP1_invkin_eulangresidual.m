...
function [q, Phi] = S3RRP1_invkin_eulangresidual(xE_soll, q0, s)
...
for rr = 0:retry_limit % Schleife über Neu-Anfänge der Berechnung
  ...
  for jj = 2:n_max % Schleife über iteratives Annähern mit Newton-Verfahren
    ...
    % Gradienten-Matrix
    Jdk_voll = S3RRP1_constr2grad(q1, xE_soll, pkin, T_N_E, phiconv_W_E, I_EElink, reci);
    % Zwangsbedingungen
    Phi_voll = S3RRP1_constr2(q1, xE_soll, pkin, T_N_E, phiconv_W_E, I_EElink, reci);
