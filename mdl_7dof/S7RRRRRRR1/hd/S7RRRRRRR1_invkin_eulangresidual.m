...
function [q, Phi] = S7RRRRRRR1_invkin_eulangresidual(xE_soll, q0, s)

%% Coder Information
%#codegen
%$cgargs {zeros(6,1),zeros(7,1),struct(
...
for rr = 0:retry_limit % Schleife über Neu-Anfänge der Berechnung
  ...
  for jj = 2:n_max % Schleife über iteratives Annähern mit Newton-Verfahren
    ...
    % Gradienten-Matrix
    Jdk_voll = S7RRRRRRR1_constr2grad(q1, xE_soll, pkin, T_N_E, phiconv_W_E, I_EElink, reci);
    % Zwangsbedingungen
    Phi_voll = S7RRRRRRR1_constr2(q1, xE_soll, pkin, T_N_E, phiconv_W_E, I_EElink, reci);
