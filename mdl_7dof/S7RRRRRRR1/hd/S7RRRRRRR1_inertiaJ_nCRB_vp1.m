...
function H = S7RRRRRRR1_inertiaJ_nCRB_vp1(q, pkin, m_num, rSges_num_mdh, Icges_num_mdh)
%%Coder Information
%#codegen
%$cgargs {zeros(7,1),zeros(4,1),zeros(8,1),zeros(8,3),zeros(8,6)}
assert(isreal(q) && all(size(q) == [7 1]), ...
  'S7RRRRRRR1_inertiaJ_nCRB_vp1: q has to be [7x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [4 1]), ...
  'S7RRRRRRR1_inertiaJ_nCRB_vp1: Kinematic parameters pkin have to be [4x1] (double)');
assert(isreal(m_num) && all(size(m_num) == [8 1]), ...
  'S7RRRRRRR1_inertiaJ_nCRB_vp1: m_num has to be [8x1] (double)');
assert(isreal(rSges_num_mdh) && all(size(rSges_num_mdh) == [8,3]), ...
  'S7RRRRRRR1_inertiaJ_nCRB_vp1: rSges_num_mdh has to be [8x3] (double)');
assert(isreal(Icges_num_mdh) && all(size(Icges_num_mdh) == [8 6]), ...
  'S7RRRRRRR1_inertiaJ_nCRB_vp1: Icges_num_mdh has to be [8x6] (double)');

%% Init
[v_mdh, sigma] = S7RRRRRRR1_structural_kinematic_parameters();
...
%% Kinematik
T_mdh = S7RRRRRRR1_joint_trafo_rotmat_mdh_sym_varpar(q, pkin);
...
I_c_ges = NaN(7,36);
for i = 1:7 % loop through all bodies, [Featherstone2008] T6.2, Line 2
...
for i = 7:-1:1 % Wird die Schleife mit uint8 definiert, funktioniert der generierte mex-Code nicht (Matlab-Bug)
  ...
  for tmp = 1:7 % Dummy-Schleife für Kompilierbarkeit. Abbruchkriterium s.u.
