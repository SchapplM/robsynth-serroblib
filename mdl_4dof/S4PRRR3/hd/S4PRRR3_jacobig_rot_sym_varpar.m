...
function Jg_rot = S4PRRR3_jacobig_rot_sym_varpar(qJ, link_index, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(4,1),uint8(0),zeros(7,1)}
assert(isreal(qJ) && all(size(qJ) == [4 1]), ...
  'S4PRRR3_jacobig_rot_sym_varpar: qJ has to be [4x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
	'S4PRRR3_jacobig_rot_sym_varpar: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [7 1]), ...
  'S4PRRR3_jacobig_rot_sym_varpar: pkin has to be [7x1] (double)');
if link_index == 0
...
elseif link_index == 4
	...
	t22 = pkin(7) + qJ(2) + qJ(3);
	t1 = [0, 0, 0, sin(t22); 0, 0, 0, -cos(t22); 0, 1, 1, 0;];
	Jg_rot = t1;
else
	Jg_rot=NaN(3,4);
end