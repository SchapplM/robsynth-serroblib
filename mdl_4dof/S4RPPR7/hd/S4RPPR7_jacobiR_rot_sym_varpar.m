...
function JR_rot = S4RPPR7_jacobiR_rot_sym_varpar(qJ, link_index, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(4,1),uint8(0),zeros(6,1)}
assert(isreal(qJ) && all(size(qJ) == [4 1]), ...
  'S4RPPR7_jacobiR_rot_sym_varpar: qJ has to be [4x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
	'S4RPPR7_jacobiR_rot_sym_varpar: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [6 1]), ...
  'S4RPPR7_jacobiR_rot_sym_varpar: pkin has to be [6x1] (double)');
if link_index == 0
...
elseif link_index == 3
	...
	t8 = cos(pkin(6));
	t7 = sin(pkin(6));
	t1 = [t10 * t7, 0, 0, 0; t9 * t7, 0, 0, 0; 0, 0, 0, 0; t10 * t8, 0, 0, 0; t9 * t8, 0, 0, 0; 0, 0, 0, 0; -t9, 0, 0, 0; t10, 0, 0, 0; 0, 0, 0, 0;];
	JR_rot = t1;
elseif link_index == 4
	%% Symbolic Calculation
	...
	t13 = t18 * t14;
	t12 = t17 * t15;
	t1 = [t13, 0, 0, t12; t20, 0, 0, -t19; 0, 0, 0, -t14; t19, 0, 0, -t20; t12, 0, 0, t13; 0, 0, 0, -t15; -t17, 0, 0, 0; t18, 0, 0, 0; 0, 0, 0, 0;];
	JR_rot = t1;
else
	JR_rot=NaN(9,4);
end