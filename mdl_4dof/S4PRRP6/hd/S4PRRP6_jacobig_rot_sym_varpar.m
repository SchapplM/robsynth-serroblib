...
function Jg_rot = S4PRRP6_jacobig_rot_sym_varpar(qJ, link_index, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(4,1),uint8(0),zeros(6,1)}
assert(isreal(qJ) && all(size(qJ) == [4 1]), ...
  'S4PRRP6_jacobig_rot_sym_varpar: qJ has to be [4x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
	'S4PRRP6_jacobig_rot_sym_varpar: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [6 1]), ...
  'S4PRRP6_jacobig_rot_sym_varpar: pkin has to be [6x1] (double)');
if link_index == 0
...
elseif link_index == 4
	...
	t58 = cos(pkin(6));
	t57 = sin(pkin(6));
	t1 = [0, t57, t58 * t59, 0; 0, -t58, t57 * t59, 0; 0, 0, -cos(qJ(2)), 0;];
	Jg_rot = t1;
else
	Jg_rot=NaN(3,4);
end