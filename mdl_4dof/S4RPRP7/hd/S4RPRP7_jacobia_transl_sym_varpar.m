...
function Ja_transl = S4RPRP7_jacobia_transl_sym_varpar(qJ, link_index, r_i_i_C, ...
  pkin)
...
assert(isreal(qJ) && all(size(qJ) == [4 1]), ...
  'S4RPRP7_jacobia_transl_sym_varpar: qJ has to be [4x1] (double)');
assert(isa(r_i_i_C,'double') && isreal(r_i_i_C) && all(size(r_i_i_C) == [3 1]), ...
	'S4RPRP7_jacobia_transl_sym_varpar: Position vector r_i_i_C has to be [3x1] double');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
	'S4RPRP7_jacobia_transl_sym_varpar: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [5 1]), ...
  'S4RPRP7_jacobia_transl_sym_varpar: pkin has to be [5x1] (double)');
if link_index == 0
...
elseif link_index == 4
	...
	t10 = -t8 * t1 + t7 * t3;
	t9 = t7 * t1 + t8 * t3;
	t6 = pkin(1) + pkin(5) + r_i_i_C(2);
	t5 = qJ(2) - t10;
	t4 = cos(qJ(1));
	t2 = sin(qJ(1));
	t11 = [-t6 * t2 + t5 * t4, t2, t9 * t2, -t2 * t3; t5 * t2 + t6 * t4, -t4, -t9 * t4, t4 * t3; 0, 0, t10, t1;];
	Ja_transl = t11;
else
	Ja_transl=NaN(3,4);
end