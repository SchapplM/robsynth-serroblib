...
function Ja_transl = S4PRRR4_jacobia_transl_sym_varpar(qJ, link_index, r_i_i_C, ...
  pkin)
...
assert(isreal(qJ) && all(size(qJ) == [4 1]), ...
  'S4PRRR4_jacobia_transl_sym_varpar: qJ has to be [4x1] (double)');
assert(isa(r_i_i_C,'double') && isreal(r_i_i_C) && all(size(r_i_i_C) == [3 1]), ...
	'S4PRRR4_jacobia_transl_sym_varpar: Position vector r_i_i_C has to be [3x1] double');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
	'S4PRRR4_jacobia_transl_sym_varpar: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [7 1]), ...
  'S4PRRR4_jacobia_transl_sym_varpar: pkin has to be [7x1] (double)');
if link_index == 0
	%% Symbolic Calculation
	% From jacobia_transl_0_floatb_twist_matlab.m
	% OptimizationMode: 2
	% StartTime: 2019-12-29 12:27:10
...
elseif link_index == 4
	...
	t4 = cos(t8);
	t3 = sin(t8);
	t1 = [0, -t13 * t3 + t16 * t4, t12 * t4, t14 * t4; 0, t13 * t4 + t16 * t3, t12 * t3, t14 * t3; 1, 0, t18, t15;];
	Ja_transl = t1;
else
	Ja_transl=NaN(3,4);
end