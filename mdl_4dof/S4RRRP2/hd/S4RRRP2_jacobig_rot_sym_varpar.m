...
function Jg_rot = S4RRRP2_jacobig_rot_sym_varpar(qJ, link_index, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(4,1),uint8(0),zeros(6,1)}
assert(isreal(qJ) && all(size(qJ) == [4 1]), ...
  'S4RRRP2_jacobig_rot_sym_varpar: qJ has to be [4x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
	'S4RRRP2_jacobig_rot_sym_varpar: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [6 1]), ...
  'S4RRRP2_jacobig_rot_sym_varpar: pkin has to be [6x1] (double)');
if link_index == 0
...
elseif link_index == 4
	...
	% Computational Cost: add. (3->2), mult. (0->0), div. (0->0), fcn. (2->2), ass. (0->2)
	t24 = qJ(1) + qJ(2);
	t1 = [0, 0, sin(t24), 0; 0, 0, -cos(t24), 0; 1, 1, 0, 0;];
	Jg_rot = t1;
else
	Jg_rot=NaN(3,4);
end