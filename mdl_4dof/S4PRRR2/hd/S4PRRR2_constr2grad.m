...
function Phi_dq = S4PRRR2_constr2grad(q, xE, pkin, T_N_E, phiconv_W_E, I_EElink, reci)

%% Coder Information
%#codegen
%$cgargs {zeros(4,1),zeros(6,1),zeros(2,1),zeros(4,4),uint8(2),uint8(0), true}

%% Translatorisch
% Bein-Jacobi
J0_i_trans = S4PRRR2_jacobia_transl_sym_varpar(q, I_EElink, T_N_E(1:3,4), pkin);
...
Tc_ges = S4PRRR2_fkine_fixb_rotmat_mdh_sym_varpar(q, pkin);
...
dRb_0N_dq = S4PRRR2_jacobiR_rot_sym_varpar(q, I_EElink, pkin);
