...
function [tau, v_i_i_ges, w_i_i_ges] = S2RR2_invdyn_floatb_eulxyz_nnew_vp1(q, qD, qDD, phi_base, xD_base, xDD_base, ...
  pkin, m_num, rSges_num_mdh, Icges_num_mdh)

%%Coder Information
%#codegen
%$cgargs {zeros(2,1),zeros(2,1),zeros(2,1),zeros(3,1),zeros(6,1),zeros(6,1)
%$cgargs  zeros(1,1),zeros(3,1),zeros(3,3),zeros(3,6)}
assert(isreal(q) && all(size(q) == [2 1]), ...
  'S2RR2_invdyn_floatb_eulxyz_nnew_vp1: q has to be [2x1] (double)');
assert(isreal(qD) && all(size(qD) == [2 1]), ...
  'S2RR2_invdyn_floatb_eulxyz_nnew_vp1: qD has to be [2x1] (double)');
assert(isreal(qDD) && all(size(qDD) == [2 1]), ...
  'S2RR2_invdyn_floatb_eulxyz_nnew_vp1: qDD has to be [2x1] (double)');
assert(isreal(phi_base) && all(size(phi_base) == [3 1]), ...
  'S2RR2_invdyn_floatb_eulxyz_nnew_vp1: phi_base has to be [3x1] (double)');
assert(isreal(xD_base) && all(size(xD_base) == [6 1]), ...
  'S2RR2_invdyn_floatb_eulxyz_nnew_vp1: xD_base has to be [6x1] (double)');
assert(isreal(xDD_base) && all(size(xDD_base) == [6 1]), ...
  'S2RR2_invdyn_floatb_eulxyz_nnew_vp1: xDD_base has to be [6x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [1 1]), ...
  'S2RR2_invdyn_floatb_eulxyz_nnew_vp1: Kinematic parameters pkin have to be [1x1] (double)');
assert(isreal(m_num) && all(size(m_num) == [3 1]), ...
  'S2RR2_invdyn_floatb_eulxyz_nnew_vp1: m_num has to be [3x1] (double)');
assert(isreal(rSges_num_mdh) && all(size(rSges_num_mdh) == [3,3]), ...
  'S2RR2_invdyn_floatb_eulxyz_nnew_vp1: rSges_num_mdh has to be [3x3] (double)');
assert(isreal(Icges_num_mdh) && all(size(Icges_num_mdh) == [3 6]), ...
  'S2RR2_invdyn_floatb_eulxyz_nnew_vp1: Icges_num_mdh has to be [3x6] (double)');

%% Init
tau_J = NaN(2,1);
R_W_0 = eulxyz2r(phi_base);
T_basevel = eulxyzjac(phi_base);
[v_mdh, sigma] = S2RR2_structural_kinematic_parameters();


%% Vorwärts-Iteration
% Positionen. Berechne symbolisch (effizienter).
T_mdh = S2RR2_joint_trafo_rotmat_mdh_sym_varpar(q, pkin);
