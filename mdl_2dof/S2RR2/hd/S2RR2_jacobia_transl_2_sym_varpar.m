...
function Ja_transl = S2RR2_jacobia_transl_2_sym_varpar(qJ, r_i_i_C, ...
  pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(2,1),zeros(3,1),zeros(1,1)}
assert(isreal(qJ) && all(size(qJ) == [2 1]), ...
  'S2RR2_jacobia_transl_2_sym_varpar: qJ has to be [2x1] (double)');
assert(isa(r_i_i_C,'double') && isreal(r_i_i_C) && all(size(r_i_i_C) == [3 1]), ...
	'S2RR2_jacobia_transl_2_sym_varpar: Position vector r_i_i_C has to be [3x1] double');
assert(isreal(pkin) && all(size(pkin) == [1 1]), ...
  'S2RR2_jacobia_transl_2_sym_varpar: pkin has to be [1x1] (double)');
...
t4 = cos(qJ(1));
t2 = sin(qJ(1));
t8 = [-t2 * t6 + t4 * t7, -t5 * t4; 0, t6; -t2 * t7 - t4 * t6, t5 * t2;];
Ja_transl  = t8;
