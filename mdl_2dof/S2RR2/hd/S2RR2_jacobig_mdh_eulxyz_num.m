...
function Jg_C = S2RR2_jacobig_mdh_eulxyz_num(phi_base, qJ, link_index, r_i_i_C, pkin)
%% Init
%#codegen
%$cgargs {zeros(3,1),zeros(2,1),uint8(zeros(1,1)),zeros(3,1),zeros(1,1)}
assert(isreal(phi_base) && all(size(phi_base) == [3 1]), ...
  'S2RR2_jacobig_mdh_eulxyz_num: Base RPY angles have to be [3x1] (double)');
assert(isreal(qJ) && all(size(qJ) == [2 1]), ...
  'S2RR2_jacobig_mdh_eulxyz_num: Joint angles qJ have to be [2x1] (double)');
assert(isreal(r_i_i_C) && all(size(r_i_i_C) == [3 1]), ...
  'S2RR2_jacobig_mdh_eulxyz_num: Position vector r_i_i_C has to be [3x1] (double)');
assert(all(size(link_index) == [1 1]), ...
  'S2RR2_jacobig_mdh_eulxyz_num: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [1 1]), ...
  'S2RR2_jacobig_mdh_eulxyz_num: Kinematic parameters pkin have to be [1x1] (double)');
