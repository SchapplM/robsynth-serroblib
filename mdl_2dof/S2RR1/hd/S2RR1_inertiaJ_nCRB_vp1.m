...
function H = S2RR1_inertiaJ_nCRB_vp1(q, pkin, m_num, rSges_num_mdh, Icges_num_mdh)
%%Coder Information
%#codegen
%$cgargs {zeros(2,1),zeros(1,1),zeros(3,1),zeros(3,3),zeros(3,6)}
assert(isreal(q) && all(size(q) == [2 1]), ...
  'S2RR1_inertiaJ_nCRB_vp1: q has to be [2x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [1 1]), ...
  'S2RR1_inertiaJ_nCRB_vp1: Kinematic parameters pkin have to be [1x1] (double)');
assert(isreal(m_num) && all(size(m_num) == [3 1]), ...
  'S2RR1_inertiaJ_nCRB_vp1: m_num has to be [3x1] (double)');
assert(isreal(rSges_num_mdh) && all(size(rSges_num_mdh) == [3,3]), ...
  'S2RR1_inertiaJ_nCRB_vp1: rSges_num_mdh has to be [3x3] (double)');
assert(isreal(Icges_num_mdh) && all(size(Icges_num_mdh) == [3 6]), ...
  'S2RR1_inertiaJ_nCRB_vp1: Icges_num_mdh has to be [3x6] (double)');
