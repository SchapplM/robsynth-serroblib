...
t57 = cos(qJ(1));
t40 = -t45 * t57 - t46 * t54;
t42 = -t54 * V_base(5) + t57 * V_base(6);
t48 = V_base(4) + qJD(1);
t33 = pkin(1) * t48 - pkin(6) * t42 + t40;
t39 = -t45 * t54 + t57 * t46;
t43 = -t54 * V_base(6) - t57 * V_base(5);
t36 = pkin(6) * t43 + t39;
t53 = sin(qJ(2));
t56 = cos(qJ(2));
t25 = t56 * t33 - t36 * t53;
...
t2 = t3 * t52 + t4 * t55;
t1 = t3 * t55 - t4 * t52;
t8 = m(2) * (t39 ^ 2 + t40 ^ 2 + t58) / 0.2e1 + m(1) * (V_base(2) ^ 2 + V_base(3) ^ 2 + t58) / 0.2e1 + m(3) * (t25 ^ 2 + t26 ^ 2 + t41 ^ 2) / 0.2e1 + m(4) * (t11 ^ 2 + t12 ^ 2 + t30 ^ 2) / 0.2e1 + m(6) * (t1 ^ 2 + t2 ^ 2 + t7 ^ 2) / 0.2e1 + m(5) * (t5 ^ 2 + t6 ^ 2 + t9 ^ 2) / 0.2e1 + (V_base(2) * mrSges(1,1) - V_base(1) * mrSges(1,2) + Ifges(1,3) * V_base(6) / 0.2e1) * V_base(6) + (t40 * mrSges(2,1) - t39 * mrSges(2,2) + Ifges(2,3) * t48 / 0.2e1) * t48 + (t41 * mrSges(3,2) - t25 * mrSges(3,3) + Ifges(3,1) * t38 / 0.2e1) * t38 + (t30 * mrSges(4,2) - t11 * mrSges(4,3) + Ifges(4,1) * t29 / 0.2e1) * t29 + (t1 * mrSges(6,1) - t2 * mrSges(6,2) + Ifges(6,3) * t27 / 0.2e1) * t27 + (t9 * mrSges(5,2) - t5 * mrSges(5,3) + Ifges(5,1) * t24 / 0.2e1) * t24 + (-V_base(3) * mrSges(1,1) + V_base(1) * mrSges(1,3) + Ifges(1,6) * V_base(6) + Ifges(1,2) * V_base(5) / 0.2e1) * V_base(5) + (-V_base(1) * mrSges(2,1) + t39 * mrSges(2,3) + Ifges(2,6) * t48 + Ifges(2,2) * t43 / 0.2e1) * t43 + (-t41 * mrSges(3,1) + t26 * mrSges(3,3) + Ifges(3,4) * t38 + Ifges(3,2) * t37 / 0.2e1) * t37 + (-t9 * mrSges(5,1) + t6 * mrSges(5,3) + Ifges(5,4) * t24 + Ifges(5,2) * t23 / 0.2e1) * t23 + (t7 * mrSges(6,2) - t1 * mrSges(6,3) + Ifges(6,5) * t27 + Ifges(6,1) * t17 / 0.2e1) * t17 + (V_base(3) * mrSges(1,2) - V_base(2) * mrSges(1,3) + Ifges(1,4) * V_base(5) + Ifges(1,5) * V_base(6) + Ifges(1,1) * V_base(4) / 0.2e1) * V_base(4) + (V_base(1) * mrSges(2,2) - t40 * mrSges(2,3) + Ifges(2,4) * t43 + Ifges(2,5) * t48 + Ifges(2,1) * t42 / 0.2e1) * t42 + (-t7 * mrSges(6,1) + t2 * mrSges(6,3) + Ifges(6,4) * t17 + Ifges(6,6) * t27 + Ifges(6,2) * t16 / 0.2e1) * t16 + (t25 * mrSges(3,1) + t11 * mrSges(4,1) - t26 * mrSges(3,2) - t12 * mrSges(4,2) + Ifges(3,5) * t38 + Ifges(4,5) * t29 + Ifges(3,6) * t37 + (Ifges(3,3) / 0.2e1 + Ifges(4,3) / 0.2e1) * t47) * t47 + (t30 * mrSges(4,1) + t5 * mrSges(5,1) - t6 * mrSges(5,2) - t12 * mrSges(4,3) - Ifges(4,4) * t29 + Ifges(5,5) * t24 - Ifges(4,6) * t47 + Ifges(5,6) * t23 + (Ifges(4,2) / 0.2e1 + Ifges(5,3) / 0.2e1) * t28) * t28;
T = t8;
