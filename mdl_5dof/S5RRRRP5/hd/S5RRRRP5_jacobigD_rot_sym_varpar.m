...
function JgD_rot = S5RRRRP5_jacobigD_rot_sym_varpar(qJ, qJD, link_index, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(5,1),zeros(5,1),uint8(0),zeros(8,1)}
assert(isreal(qJ) && all(size(qJ) == [5 1]), ...
  'S5RRRRP5_jacobigD_rot_sym_varpar: qJ has to be [5x1] (double)');
assert(isreal(qJD) && all(size(qJD) == [5 1]), ...
  'S5RRRRP5_jacobigD_rot_sym_varpar: qJD has to be [5x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
	'S5RRRRP5_jacobigD_rot_sym_varpar: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [8 1]), ...
  'S5RRRRP5_jacobigD_rot_sym_varpar: pkin has to be [8x1] (double)');
if link_index == 0
...
elseif link_index == 5
	...
	t81 = qJD(1) * cos(qJ(1));
	t80 = qJD(1) * sin(qJ(1));
	t1 = [0, t81, t81, t81, 0; 0, t80, t80, t80, 0; 0, 0, 0, 0, 0;];
	JgD_rot = t1;
else
	JgD_rot=NaN(3,5);
end