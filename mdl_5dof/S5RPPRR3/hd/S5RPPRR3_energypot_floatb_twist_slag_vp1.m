...
t8 = cos(t15);
t6 = sin(t15);
t1 = -m(1) * (g(1) * (r_base(1) + rSges(1,1)) + g(2) * (r_base(2) + rSges(1,2)) + g(3) * (r_base(3) + rSges(1,3))) - m(2) * (g(1) * (rSges(2,3) + t29) + g(2) * (-t19 * rSges(2,1) - rSges(2,2) * t20 + r_base(2)) + g(3) * (rSges(2,1) * t20 - t19 * rSges(2,2) + r_base(3))) - m(3) * (g(1) * (rSges(3,3) + t27) + g(2) * (-rSges(3,1) * t6 - rSges(3,2) * t8 + t26) + g(3) * (rSges(3,1) * t8 - rSges(3,2) * t6 + t28)) - m(4) * (g(1) * (rSges(4,1) * t16 + rSges(4,2) * t17 + t27) + (g(2) * t30 + g(3) * t22) * t8 + (-g(2) * t22 + g(3) * t30) * t6 + t21) - m(5) * (g(1) * (rSges(5,1) * t5 + rSges(5,2) * t7 + t25) + (g(2) * t32 + g(3) * t24) * t8 + (-g(2) * t24 + g(3) * t32) * t6 + t21) - m(6) * (g(1) * (rSges(6,1) * t2 + rSges(6,2) * t3 + pkin(4) * t5 + t25) + (g(2) * t31 + g(3) * t23) * t8 + (-g(2) * t23 + g(3) * t31) * t6 + t21);
U = t1;
