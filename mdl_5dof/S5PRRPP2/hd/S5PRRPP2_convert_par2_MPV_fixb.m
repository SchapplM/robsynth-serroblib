...
t12 = [m(2) + m(3) + m(4); t11 + 0.2e1 * t8 + ((pkin(2) ^ 2 + pkin(6) ^ 2) * m(4)) + (2 * pkin(6) * mrSges(4,3)) + Ifges(3,3) + Ifges(4,2) - t9; pkin(2) * m(4) + mrSges(3,1); -pkin(6) * m(4) + mrSges(3,2) - mrSges(4,3); Ifges(4,1) - Ifges(4,2) + t1 - 0.4e1 * t8 - 0.2e1 * t11; -t1 * t10 + 0.2e1 * t2 * t7 + Ifges(4,4) - t7; -t5 * t3 + t6 * t4 + Ifges(4,5); t6 * t3 + t5 * t4 + Ifges(4,6); Ifges(4,3) + Ifges(5,3) + Ifges(6,2); mrSges(4,1); mrSges(4,2); mrSges(5,1); mrSges(5,2); mrSges(5,3); m(5); mrSges(6,1); mrSges(6,2); mrSges(6,3); m(6);];
MPV = t12;
