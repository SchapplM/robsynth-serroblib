...
t35 = -t40 * t51 - t41 * t48;
t43 = V_base(4) + qJD(1);
t29 = pkin(1) * t43 - pkin(6) * t37 + t35;
t34 = -t40 * t48 + t51 * t41;
t31 = pkin(6) * t38 + t34;
t21 = t50 * t29 - t31 * t47;
t42 = qJD(2) + t43;
t15 = pkin(2) * t42 - qJ(3) * t33 + t21;
t22 = t47 * t29 + t50 * t31;
t18 = qJ(3) * t32 + t22;
t10 = t44 * t15 + t45 * t18;
t8 = pkin(7) * t42 + t10;
...
t6 = m(2) * (t34 ^ 2 + t35 ^ 2 + t52) / 0.2e1 + m(1) * (V_base(2) ^ 2 + V_base(3) ^ 2 + t52) / 0.2e1 + m(3) * (t21 ^ 2 + t22 ^ 2 + t36 ^ 2) / 0.2e1 + m(4) * (t10 ^ 2 + t26 ^ 2 + t9 ^ 2) / 0.2e1 + m(6) * (t1 ^ 2 + t2 ^ 2 + t5 ^ 2) / 0.2e1 + m(5) * (t3 ^ 2 + t4 ^ 2 + t7 ^ 2) / 0.2e1 + (V_base(2) * mrSges(1,1) - V_base(1) * mrSges(1,2) + Ifges(1,3) * V_base(6) / 0.2e1) * V_base(6) + (t35 * mrSges(2,1) - t34 * mrSges(2,2) + Ifges(2,3) * t43 / 0.2e1) * t43 + (t36 * mrSges(3,2) - t21 * mrSges(3,3) + Ifges(3,1) * t33 / 0.2e1) * t33 + (t26 * mrSges(4,2) - t9 * mrSges(4,3) + Ifges(4,1) * t25 / 0.2e1) * t25 + (-V_base(3) * mrSges(1,1) + V_base(1) * mrSges(1,3) + Ifges(1,6) * V_base(6) + Ifges(1,2) * V_base(5) / 0.2e1) * V_base(5) + (-V_base(1) * mrSges(2,1) + t34 * mrSges(2,3) + Ifges(2,6) * t43 + Ifges(2,2) * t38 / 0.2e1) * t38 + (-t36 * mrSges(3,1) + t22 * mrSges(3,3) + Ifges(3,4) * t33 + Ifges(3,2) * t32 / 0.2e1) * t32 + (-t26 * mrSges(4,1) + t10 * mrSges(4,3) + Ifges(4,4) * t25 + Ifges(4,2) * t24 / 0.2e1) * t24 + (V_base(3) * mrSges(1,2) - V_base(2) * mrSges(1,3) + Ifges(1,4) * V_base(5) + Ifges(1,5) * V_base(6) + Ifges(1,1) * V_base(4) / 0.2e1) * V_base(4) + (V_base(1) * mrSges(2,2) - t35 * mrSges(2,3) + Ifges(2,4) * t38 + Ifges(2,5) * t43 + Ifges(2,1) * t37 / 0.2e1) * t37 + (t3 * mrSges(5,1) + t1 * mrSges(6,1) - t4 * mrSges(5,2) - t2 * mrSges(6,2) + (Ifges(5,3) / 0.2e1 + Ifges(6,3) / 0.2e1) * t23) * t23 + (t7 * mrSges(5,2) + t5 * mrSges(6,2) - t3 * mrSges(5,3) - t1 * mrSges(6,3) + (Ifges(6,1) / 0.2e1 + Ifges(5,1) / 0.2e1) * t20 + (Ifges(5,5) + Ifges(6,5)) * t23) * t20 + (t21 * mrSges(3,1) + t9 * mrSges(4,1) - t22 * mrSges(3,2) - t10 * mrSges(4,2) + Ifges(3,5) * t33 + Ifges(4,5) * t25 + Ifges(3,6) * t32 + Ifges(4,6) * t24 + (Ifges(3,3) / 0.2e1 + Ifges(4,3) / 0.2e1) * t42) * t42 + (-t7 * mrSges(5,1) - t5 * mrSges(6,1) + t4 * mrSges(5,3) + t2 * mrSges(6,3) + (Ifges(6,2) / 0.2e1 + Ifges(5,2) / 0.2e1) * t19 + (Ifges(5,6) + Ifges(6,6)) * t23 + (Ifges(5,4) + Ifges(6,4)) * t20) * t19;
T = t6;
