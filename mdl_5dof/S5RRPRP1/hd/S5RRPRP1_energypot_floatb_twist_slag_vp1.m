...
t2 = cos(t5);
t1 = sin(t5);
t3 = -m(1) * (g(1) * (r_base(1) + rSges(1,1)) + g(2) * (r_base(2) + rSges(1,2)) + g(3) * (r_base(3) + rSges(1,3))) - m(2) * (g(1) * (rSges(2,3) + t24) + g(2) * (t12 * rSges(2,1) + rSges(2,2) * t14 + r_base(2)) + g(3) * (-rSges(2,1) * t14 + t12 * rSges(2,2) + r_base(3))) - m(3) * (g(1) * (rSges(3,3) + t22) + g(2) * (rSges(3,1) * t6 + rSges(3,2) * t7 + t23) + g(3) * (-t7 * rSges(3,1) + t6 * rSges(3,2) + t20)) - m(4) * (g(1) * (rSges(4,3) + t19) + g(2) * (rSges(4,1) * t1 + rSges(4,2) * t2 + t21) + g(3) * (-t2 * rSges(4,1) + t1 * rSges(4,2) + t16)) - m(5) * (g(1) * (rSges(5,1) * t11 + rSges(5,2) * t13 + t19) + (g(2) * t26 - g(3) * t18) * t2 + (g(2) * t18 + g(3) * t26) * t1 + t15) - m(6) * (g(1) * (rSges(6,2) * t13 + t27 * t11 + t19) + (g(2) * t25 - g(3) * t17) * t2 + (g(2) * t17 + g(3) * t25) * t1 + t15);
U = t3;
