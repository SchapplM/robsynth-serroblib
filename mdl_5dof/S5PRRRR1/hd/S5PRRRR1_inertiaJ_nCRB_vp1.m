...
function H = S5PRRRR1_inertiaJ_nCRB_vp1(q, pkin, m_num, rSges_num_mdh, Icges_num_mdh)
%%Coder Information
%#codegen
%$cgargs {zeros(5,1),zeros(2,1),zeros(6,1),zeros(6,3),zeros(6,6)}
assert(isreal(q) && all(size(q) == [5 1]), ...
  'S5PRRRR1_inertiaJ_nCRB_vp1: q has to be [5x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [2 1]), ...
  'S5PRRRR1_inertiaJ_nCRB_vp1: Kinematic parameters pkin have to be [2x1] (double)');
assert(isreal(m_num) && all(size(m_num) == [6 1]), ...
  'S5PRRRR1_inertiaJ_nCRB_vp1: m_num has to be [6x1] (double)');
assert(isreal(rSges_num_mdh) && all(size(rSges_num_mdh) == [6,3]), ...
  'S5PRRRR1_inertiaJ_nCRB_vp1: rSges_num_mdh has to be [6x3] (double)');
assert(isreal(Icges_num_mdh) && all(size(Icges_num_mdh) == [6 6]), ...
  'S5PRRRR1_inertiaJ_nCRB_vp1: Icges_num_mdh has to be [6x6] (double)');

%% Init
[v_mdh, sigma] = S5PRRRR1_structural_kinematic_parameters();
...
%% Kinematik
T_mdh = S5PRRRR1_joint_trafo_rotmat_mdh_sym_varpar(q, pkin);
