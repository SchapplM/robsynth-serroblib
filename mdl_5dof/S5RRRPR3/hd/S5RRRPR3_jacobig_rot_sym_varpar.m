...
function Jg_rot = S5RRRPR3_jacobig_rot_sym_varpar(qJ, link_index, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(5,1),uint8(0),zeros(9,1)}
assert(isreal(qJ) && all(size(qJ) == [5 1]), ...
  'S5RRRPR3_jacobig_rot_sym_varpar: qJ has to be [5x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
	'S5RRRPR3_jacobig_rot_sym_varpar: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [9 1]), ...
  'S5RRRPR3_jacobig_rot_sym_varpar: pkin has to be [9x1] (double)');
if link_index == 0
...
elseif link_index == 4
	...
	% Computational Cost: add. (2->1), mult. (0->0), div. (0->0), fcn. (2->2), ass. (0->2)
	t38 = qJ(1) + qJ(2);
	t1 = [1, 1, 0, 0, 0; 0, 0, cos(t38), 0, 0; 0, 0, sin(t38), 0, 0;];
	Jg_rot = t1;
elseif link_index == 5
	%% Symbolic Calculation
	...
	t46 = cos(t47);
	t45 = sin(t47);
	t1 = [1, 1, 0, 0, 0; 0, 0, t46, 0, t46; 0, 0, t45, 0, t45;];
	Jg_rot = t1;
else
	Jg_rot=NaN(3,5);
end