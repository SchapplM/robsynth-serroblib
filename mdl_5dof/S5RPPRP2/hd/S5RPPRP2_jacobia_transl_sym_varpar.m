...
function Ja_transl = S5RPPRP2_jacobia_transl_sym_varpar(qJ, link_index, r_i_i_C, ...
  pkin)
...
assert(isreal(qJ) && all(size(qJ) == [5 1]), ...
  'S5RPPRP2_jacobia_transl_sym_varpar: qJ has to be [5x1] (double)');
assert(isa(r_i_i_C,'double') && isreal(r_i_i_C) && all(size(r_i_i_C) == [3 1]), ...
	'S5RPPRP2_jacobia_transl_sym_varpar: Position vector r_i_i_C has to be [3x1] double');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
	'S5RPPRP2_jacobia_transl_sym_varpar: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [8 1]), ...
  'S5RPPRP2_jacobia_transl_sym_varpar: pkin has to be [8x1] (double)');
if link_index == 0
	%% Symbolic Calculation
	% From jacobia_transl_0_floatb_twist_matlab.m
	% OptimizationMode: 2
	% StartTime: 2019-12-29 15:56:44
...
elseif link_index == 5
	...
	t5 = cos(t7);
	t3 = sin(t7);
	t1 = [-sin(qJ(1)) * pkin(1) + t12 * t5 - t14 * t3, 0, t3, t9 * t5, t5 * t2; cos(qJ(1)) * pkin(1) + t12 * t3 + t14 * t5, 0, -t5, t9 * t3, t3 * t2; 0, 1, 0, t10, -t4;];
	Ja_transl = t1;
else
	Ja_transl=NaN(3,5);
end