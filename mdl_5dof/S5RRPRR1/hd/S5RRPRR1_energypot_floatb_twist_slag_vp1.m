...
t22 = t13 * t15;
t21 = rSges(4,3) + qJ(3);
t20 = t14 * t22 + r_base(1);
t10 = sin(qJ(2));
t19 = t10 * t15 + r_base(3);
t8 = -pkin(3) - qJ(3);
t18 = t11 * t22 + t14 * t8 + r_base(2);
t17 = rSges(3,1) * t13 - rSges(3,2) * t10;
t16 = -rSges(4,2) * t10 + t13 * t32;
t1 = -m(1) * (g(1) * (r_base(1) + rSges(1,1)) + g(2) * (r_base(2) + rSges(1,2)) + g(3) * (r_base(3) + rSges(1,3))) - m(2) * (g(1) * (rSges(2,1) * t14 - rSges(2,2) * t11 + r_base(1)) + g(2) * (rSges(2,1) * t11 + rSges(2,2) * t14 + r_base(2)) + g(3) * (r_base(3) + rSges(2,3))) - m(3) * (g(1) * (rSges(3,3) * t11 + t14 * t17 + r_base(1)) + g(2) * (-rSges(3,3) * t14 + t11 * t17 + r_base(2)) + g(3) * (rSges(3,1) * t10 + rSges(3,2) * t13 + r_base(3))) - m(4) * (g(1) * r_base(1) + g(2) * r_base(2) + g(3) * (rSges(4,2) * t13 + t10 * t32 + r_base(3)) + (g(1) * t16 - g(2) * t21) * t14 + (g(1) * t21 + g(2) * t16) * t11) - m(5) * (g(1) * (t30 * t14 + t20) + g(2) * (-rSges(5,3) * t14 + t18) + g(3) * (rSges(5,1) * t5 + rSges(5,2) * t6 + t19) + (g(1) * (rSges(5,3) - t8) + g(2) * t30) * t11) - m(6) * (g(1) * (-t11 * t8 + (t23 * t6 + t27) * rSges(6,1) + (-t26 * t6 + t24) * rSges(6,2) + t20) + g(2) * ((t24 * t6 - t26) * rSges(6,1) + (-t27 * t6 - t23) * rSges(6,2) + t18) + g(3) * (-t31 * t6 + t19) + (g(3) * (rSges(6,1) * t12 - rSges(6,2) * t9) + (g(1) * t14 + g(2) * t11) * t31) * t5);
U  = t1;
