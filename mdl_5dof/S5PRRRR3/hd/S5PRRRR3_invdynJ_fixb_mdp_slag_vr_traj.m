...
function TAU = S5PRRRR3_invdynJ_fixb_mdp_slag_vr_traj(RV_Traj, MDP)

%% Coder Information
%#codegen
%$cgargs {coder.newtype('double',[inf,NOTDEFINED]), zeros(NOTDEFINED,1)}
assert(isreal(RV_Traj) && all(size(RV_Traj,2) == NOTDEFINED), ...
  'S5PRRRR3_invdynJ_fixb_mdp_slag_vr_traj: RV_Traj needs to be [NTxNOTDEFINED] (double)');
assert(isreal(MDP) && all(size(MDP) == [NOTDEFINED 1]), ...
  'S5PRRRR3_invdynJ_fixb_mdp_slag_vr_traj: Dynamics parameter vector MDP has to be [NOTDEFINEDx1] (double)');

%% Inverse Dynamik für jeden Zeitschritt der Trajektorie berechnen
TAU = NaN(size(RV_Traj,1), 5);
for ii = 1:size(RV_Traj,1)
  TAU(ii,:) = S5PRRRR3_invdynJ_fixb_mdp_slag_vr(RV_Traj(ii,:), MDP);
end
