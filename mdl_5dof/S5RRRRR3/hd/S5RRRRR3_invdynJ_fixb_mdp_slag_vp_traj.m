...
function TAU = S5RRRRR3_invdynJ_fixb_mdp_slag_vp_traj(Q, QD, QDD, g, pkin, MDP)
...
assert(isreal(QD) && all(size(QD,2) == 5), ...
  'S5RRRRR3_invdynJ_fixb_mdp_slag_vp_traj: QD needs to be [NTx5] (double)');
assert(isreal(QDD) && all(size(QDD,2) == 5), ...
  'S5RRRRR3_invdynJ_fixb_mdp_slag_vp_traj: QDD needs to be [NTx5] (double)');
...
TAU = NaN(size(Q));
for k = 1:size(Q,1)
  TAU(k,:) = S5RRRRR3_invdynJ_fixb_mdp_slag_vp(Q(k,:)', QD(k,:)', QDD(k,:)', g, pkin, MDP);
end
