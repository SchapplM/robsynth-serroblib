...
function JgD_rot = S5PRRRR2_jacobigD_rot_4_sym_varpar(qJ, qJD, ...
  pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(5,1),zeros(5,1),zeros(1,1)}
assert(isreal(qJ) && all(size(qJ) == [5 1]), ...
  'S5PRRRR2_jacobigD_rot_4_sym_varpar: qJ has to be [5x1] (double)');
assert(isreal(qJD) && all(size(qJD) == [5 1]), ...
  'S5PRRRR2_jacobigD_rot_4_sym_varpar: qJD has to be [5x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [1 1]), ...
  'S5PRRRR2_jacobigD_rot_4_sym_varpar: pkin has to be [1x1] (double)');
...
t27 = qJD(2) * cos(qJ(2));
t26 = qJD(2) * sin(qJ(2));
t1 = [0, 0, t27, t27, 0; 0, 0, 0, 0, 0; 0, 0, t26, t26, 0;];
JgD_rot  = t1;
