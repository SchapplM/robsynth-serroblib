...
assert(isreal(rSges_num_mdh) && all(size(rSges_num_mdh) == [6,3]), ...
  'S5PRRRR2_inertiaJ_nCRB_vp1: rSges_num_mdh has to be [6x3] (double)');
assert(isreal(Icges_num_mdh) && all(size(Icges_num_mdh) == [6 6]), ...
  'S5PRRRR2_inertiaJ_nCRB_vp1: Icges_num_mdh has to be [6x6] (double)');

%% Init
[v_mdh, sigma] = S5PRRRR2_structural_kinematic_parameters();
...
%% Kinematik
T_mdh = S5PRRRR2_joint_trafo_rotmat_mdh_sym_varpar(q, pkin);
