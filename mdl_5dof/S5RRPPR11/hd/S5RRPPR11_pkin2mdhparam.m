...
function [beta_mdh, b_mdh, alpha_mdh, a_mdh, theta_mdh, d_mdh, qoffset_mdh] = S5RRPPR11_pkin2mdhparam(pkin)

%% Init
%#codegen
%$cgargs {zeros(8,1)}
assert(isreal(pkin) && all(size(pkin) == [8 1]), ...
  'S5RRPPR11_pkin2mdhparam: Kinematic parameters pkin have to be [8x1] (double)');
...
t1 = [0; 0; pi / 0.2e1; pkin(8); 0;];
theta_mdh = t1;

% Aus parameters_mdh_d_matlab.m
t1 = [pkin(5); pkin(6); 0; 0; pkin(7);];
d_mdh = t1;
