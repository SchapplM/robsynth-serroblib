...
function [beta_mdh, b_mdh, alpha_mdh, a_mdh, theta_mdh, d_mdh, qoffset_mdh] = S5PRPRR8_pkin2mdhparam(pkin)

%% Init
%#codegen
%$cgargs {zeros(9,1)}
assert(isreal(pkin) && all(size(pkin) == [9 1]), ...
  'S5PRPRR8_pkin2mdhparam: Kinematic parameters pkin have to be [9x1] (double)');
...
t1 = [pkin(9); 0; pi / 0.2e1; 0; 0;];
theta_mdh = t1;

% Aus parameters_mdh_d_matlab.m
t1 = [0; pkin(6); 0; pkin(7); pkin(8);];
d_mdh = t1;
