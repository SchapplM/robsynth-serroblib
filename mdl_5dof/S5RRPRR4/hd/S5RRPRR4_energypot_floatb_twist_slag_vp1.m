...
t2 = cos(t5);
t1 = sin(t5);
t3 = -m(1) * (g(1) * (r_base(1) + rSges(1,1)) + g(2) * (r_base(2) + rSges(1,2)) + g(3) * (r_base(3) + rSges(1,3))) - m(2) * (g(1) * (rSges(2,3) + t27) + g(2) * (-rSges(2,1) * t14 - rSges(2,2) * t16 + r_base(2)) + g(3) * (rSges(2,1) * t16 - rSges(2,2) * t14 + r_base(3))) - m(3) * (g(1) * (rSges(3,3) + t25) + g(2) * (-rSges(3,1) * t7 - rSges(3,2) * t9 + t23) + g(3) * (rSges(3,1) * t9 - rSges(3,2) * t7 + t26)) - m(4) * (g(1) * (rSges(4,3) + t22) + g(2) * (-rSges(4,1) * t1 - rSges(4,2) * t2 + t19) + g(3) * (rSges(4,1) * t2 - rSges(4,2) * t1 + t24)) - m(5) * (g(1) * (rSges(5,1) * t13 + rSges(5,2) * t15 + t22) + (g(2) * t29 + g(3) * t20) * t2 + (-g(2) * t20 + g(3) * t29) * t1 + t18) - m(6) * (g(1) * (rSges(6,1) * t6 + rSges(6,2) * t8 + pkin(4) * t13 + t22) + (g(2) * t28 + g(3) * t21) * t2 + (-g(2) * t21 + g(3) * t28) * t1 + t18);
U = t3;
